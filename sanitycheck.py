...
def run_sanity_check(test_dir):
    print("This script will perform a sanity test to ensure"
          "your code meets the criteriain the rubric.\n")
    ...
    if not test_functions_for_get:
        ...
        print(FAIL_COLOR + "\nPlease make sure you have a test case for the \
                            GET method.This MUST test both the status code  \
                            as well asthe contents of the request object.\n")
    ...
    if not test_functions_for_post:
        ...
        if not TEST_FOR_POST_METHOD_RESPONSE_BODY:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR + "One or more of your test cases for \
             POST() do not seem to be testing the contents of the response.\n")
        ...
        if len(test_functions_for_post) >= 2 and \
           COUNT_POST_METHOD_TEST_FOR_INFERENCE_RESULT < 2:
            ...
            test cases,each possible prediction that your model can make.")
...
if __name__ == "__main__":
    ...
    parser.add_argument('test_dir', metavar='test_dir', nargs='?',
                        default='tests', help='Name of the directory')
